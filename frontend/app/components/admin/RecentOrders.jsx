    ...
    <div className="bg-(--color-card) rounded-xl shadow-sm border border-badge/30 overflow-hidden">
      ...
      <div className="divide-y divide-badge/20">
        {orders.map((order) => {
          ...
          const OrderIcon =
            STATUS_ICONS[order.orderStatus?.toLowerCase()] || HelpCircle;
          const PaymentIcon =
            STATUS_ICONS[order.paymentStatus?.toLowerCase()] || HelpCircle;
              ...
              <div className="flex items-center gap-4">
                ...
                <span
                  className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${
                    STATUS_COLORS[order.orderStatus?.toLowerCase()] ||
                    "bg-gray-100"
                  }`}
                >
                  {order.orderStatus}
