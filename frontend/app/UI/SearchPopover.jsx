  ...
  return (
    ...
    >
      <div className="bg-background border border-gray-200 dark:border-primary rounded-lg z-50 max-h-80 overflow-auto">
        {isLoading && <div className="px-4 py-3 text-gray-500">Loading...</div>}

        {isError && (
          <div className="px-4 py-3 text-red-500">Error loading results</div>
        )}

        {!isLoading && !isError && results?.data.length > 0
          ? results.data.map((product, i) => (
              <ProductSearchItem product={product} key={product._id} />
            ))
          : !isLoading &&
            !isError &&
            query.length >= 3 && (
